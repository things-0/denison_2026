...
from .constants import FIG_SIZE, LINEWIDTH, H_ALPHA, H_BETA
...
def apply_poly_fit(
    ...
    if plot_adjusted:
        if blur_before_resampling:
            adjusted_plot_title = f"Spectra {baseline_year}, {year_to_adjust} (blurred then resampled to {baseline_year} grid and with polynomial fitting)"
        else:
            adjusted_plot_title = f"Spectra {baseline_year}, {year_to_adjust} (resampled to {baseline_year} grid then blurred and with polynomial fitting)"
